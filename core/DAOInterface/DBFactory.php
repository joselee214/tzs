...
class J7Factory_Db {
    ...
    {
        $adapterName = isset($config['adapter'])?$config['adapter']:null;
        if (!is_string($adapterName) or !strlen($adapterName)) {
            throw new J7Exception('Adapter name must be specified in a string');
        }

        if (!is_array($config)) {
            throw new J7Exception('Configuration must be an array');
        }

        include_once( __DIR__.'/../DBAdapter/'.$adapterName.'.php' );

        return FactoryObject::Instance($adapterName,[$config]);
    }
}
